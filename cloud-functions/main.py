...
def get_latest_article_info(request):
    try:
        ...
        if response.status_code != 200:
            return (json.dumps({"error": f"List page error {response.status_code}"}), 500, {'Content-Type': 'application/json'})
        ...
        if not urls:
            return (json.dumps({"error": "No document URLs found"}), 404, {'Content-Type': 'application/json'})

        latest_url = max(urls, key=lambda x: int(x.split("q_bbsDocNo=")[1].split("&")[0]))
        full_url = urljoin(base_url, latest_url)

        detail = requests.get(full_url, headers=headers)
        if detail.status_code != 200:
            return (json.dumps({"error": f"Detail page error {detail.status_code}"}), 500, {'Content-Type': 'application/json'})

        detail_soup = BeautifulSoup(detail.text, 'html.parser')
        content_div = detail_soup.find('div', class_='article-content')
        if not content_div:
            return (json.dumps({"error": "No article-content found"}), 404, {'Content-Type': 'application/json'})
        ...
        return (json.dumps({"weeklyInfo": lines}, ensure_ascii=False), 200, {'Content-Type': 'application/json'})
